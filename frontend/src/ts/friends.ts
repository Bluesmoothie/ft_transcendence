...
export class FriendManager
{
	...
	public refreshContainers()
	{
		...
		if (this.m_main.getId() != this.m_user.getId())
		{
	...
	{
		const	pndg: Map<User, number> = this.m_user.getPndgFriends();
		console.log(pndg)
		var		htmlUser = [];

		pndg.forEach((sender: number, friend: User) => {
			var elt: UserElement;
			if (sender != this.m_user.getId())
	...
	public addFriends(container: HTMLElement, type: UserElementType): UserElement[]
	{
		...
		const	elt: User[] = this.m_user.getFriends();
		var		htmlUser = [];
